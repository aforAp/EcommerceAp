...
const NavBar = () => {
    ...
    const {user, setUser,getCartCount, setShowUserLogin, navigate, setSearchQuery, searchQuery} = useAppContext();
    const logout = async () => {
        setUser(null);
        navigate('/');
    }
...
<div className="flex items-center gap-6 sm:hidden">
    <div onClick={() => navigate('/cart')} className="relative cursor-pointer">
            ...
            {/* Mobile Menu */}
           {open && (<div className={`${open ? 'flex' : 'hidden'} absolute top-15 left-0 w-full bg-white shadow-md py-4 flex-col items-start gap-2 px-5 text-sm md:hidden`}>
